...
async function getFeedback(endPoint: string): Promise<FeedbackModel[]> {
    const response = await my_request(endPoint);
    const responseData = response._embedded?.feedbacks || [];

    const feedbackList = responseData.map((feedback: any) => {
...
// Hàm lấy số lượng feedback chưa đọc (unread)
...
// Hàm lấy danh sách feedback chưa đọc
export async function getUnreadFeedbacks(): Promise<FeedbackModel[]> {
    try {
        const token = localStorage.getItem("token");
        if (!token) return [];
        ...
        const unreadFeedbacks = responseData.content
            .filter((feedback: any) => {
                ...
                // Những TH được coi là chưa đọc
                if (v === false) return true;        // boolean false
                if (v === "false") return true;      // string "false"
                
                return false; // còn lại coi như đã đọc
            })
            .map((feedback: any) => {
                const fb = new FeedbackModel();
                fb.idFeedback = feedback.idFeedback;
