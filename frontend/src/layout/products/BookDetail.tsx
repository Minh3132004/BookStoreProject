...
const BookDetail: React.FC<BookDetailProps> = (props) => {
	...
	const [cartList, setCartList] = useState<CartItemModel[]>([]); //Xử lý cho buy now 
	...
	const handleAddProduct = async (book: BookModel) => {
		...
		const idUser: number = Number(getIdUserByToken());
	...
	const handleBuyNow = (book: BookModel): void => {
		setCartList([
			{
				book: book,
				quantity: quantity,
